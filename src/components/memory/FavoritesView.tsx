...
export default function FavoritesView() {
  ...
  return (
    <div className="pb-8 space-y-4">
      ...
      >
        <div
          className="relative w-full rounded-[2rem] overflow-hidden bg-muted shadow-deep"
        ...
        >
          ...
          <div className="absolute top-5 right-5 w-9 h-9 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center ring-1 ring-white/15">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8">
            ...
            <p className="text-[22px] font-extrabold text-white leading-snug tracking-tight mb-4">
              {featured.content}
            </p>
            {featured.createdBy && (
          ...
          {photos[0] && (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <p className="text-[14px] font-bold text-white leading-snug">{photos[0].content}</p>
                <p className="text-[10px] text-white/50 mt-1 font-medium">{photos[0].date}</p>
              </div>
            </div>
          )}

          {/* Remaining photos in 2-column mosaic */}
          {photos.length > 1 && (
              ...
              {photos.slice(1, 5).map((item) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3">
                    ...
                    <p className="text-[9px] text-white/40 mt-0.5 font-medium">{item.date}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {milestones.map((item, i) => (
        <motion.div
          ...
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.18 + i * 0.08, duration: 0.5, ease: [0.25, 1, 0.5, 1] }}
          className="mx-4 rounded-[2rem] bg-[#FBF8F2] dark:bg-surface-raised px-8 pt-8 pb-10 shadow-card border border-stone-100/80 dark:border-stone-800/50"
        >
          <p className="text-[56px] leading-[0.7] text-amber-300 dark:text-amber-700 font-serif mb-4 select-none">&ldquo;</p>
