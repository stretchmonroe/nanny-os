...
export default function WeeklyRecap() {
  ...
  return (
    <motion.div
      ...
      className="rounded-[1.5rem] bg-sage-light border border-sage-light shadow-card overflow-hidden"
    >
      {/* Header */}
      <div className="flex items-center gap-2 px-5 pt-5 pb-4 border-b border-sage-light">
        <TrendingUp className="w-3.5 h-3.5 text-sage" strokeWidth={2.2} />
        ...
        <ul className="space-y-3">
          {observations.map((obs, i) => (
            <li key={i} className="flex items-start gap-3">
              <span className="text-sage-muted text-[16px] leading-none mt-0.5 shrink-0 select-none">—</span>
          ...
          <div className="pt-4 border-t border-sage-light">
            ...
            <div className="flex flex-wrap gap-1.5">
              {skills.map((skill) => (
                <span
                  key={skill}
                  className="text-[11px] font-semibold text-sage bg-sage-light px-3 py-1.5 rounded-full"
                >
                  {skill}
                </span>
              ))}
